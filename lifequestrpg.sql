CREATE DATABASE IF NOT EXISTS `lifequestrpg`;
USE `lifequestrpg`;
-- --------------------------------------------------------
-- Core Tables
-- --------------------------------------------------------
...
CREATE TRIGGER `after_task_complete` AFTER UPDATE ON `tasks` 
FOR EACH ROW 
BEGIN 
...
CREATE TRIGGER `after_level_up` AFTER UPDATE ON `userstats` 
FOR EACH ROW 
BEGIN 
...
CREATE TRIGGER `after_inventory_insert` AFTER INSERT ON `user_inventory` 
FOR EACH ROW 
BEGIN
